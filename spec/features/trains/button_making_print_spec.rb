...
feature 'Button Making Prints', button_making_print: true, js: true do
  ...
  context 'as an admin' do
    ...
    scenario 'I can create an order with a button making print' do
      create_order(
        name: 'Test Order',
        jobs: [
          {
      ...
      expect(page).to have_css('dd', text: 'Button making print')
    end

    context 'given an order with an imprint that is a button_making print', current: true  do
      given(:order) { create(:order, jobs: [ create(:job_with_button_making_print) ]) }
